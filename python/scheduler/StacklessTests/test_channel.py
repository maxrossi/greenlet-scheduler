...
import traceback
import contextlib
from support import SchedulerTestCase, require_one_thread
...
class TestChannels(SchedulerTestCase):
...
class TestClose(SchedulerTestCase):
...
class Subclassing(SchedulerTestCase):

    @unittest.skip('TODO')
    def test_init(self):
        """Test that we can subclass channel without overriding __new__"""
        class myclass(scheduler.channel):
