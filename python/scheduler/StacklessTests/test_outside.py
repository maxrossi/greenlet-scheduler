...
from _scheduler import _test_cframe_nr as test_cframe_nr, _test_outside as test_outside
from scheduler import tasklet, channel, run
from _testscheduler import test_cframe, test_cstate

from support import SchedulerTestCase, withThreads, get_serial_last_jump

"""
Joseph Frangoudes:
 I suspect this is testing things that the greenlet tests already cover. Keeping this around just in case, but I recon
 we'll delete this
"""
...
def current_initial_stub(threadid=-1):
    ...
    # See C source.
    return scheduler.get_thread_info(threadid, 1 << 30)[5]


MAIN_INITIAL_STUB = current_initial_stub()


class TestOutside(SchedulerTestCase):
    ...
    def test_main_exit_with_serial_mismatch(self):
        ...
        def run():
            try:
                ...
                t = tasklet(test_cstate)(scheduler.schedule_remove)
                t_cstate = t.cstate
    ...
    def test_call_from_non_main_tasklet(self):
        ...
        scheduler.tasklet(test_outside)().switch()
        self.assertTrue(done)

    def test_call_from_non_main_tasklet2(self):
        self.assertRaisesRegex(RuntimeError, "main tasklet is still scheduled", scheduler.tasklet(test_outside)().run)


class TestCframe(SchedulerTestCase):
    n = 100
