...
Joseph Frangoudes: We're going to have to rip all of the stackless-specific code out of here,
waiting to run this against the scheduler extension to figure out what we need to keep & what
we need to delete/change
"""
...
def is_zombie(tasklet):
    ...
    if not isinstance(tasklet, scheduler.tasklet):
        raise TypeError("not a tasklet")
    if tasklet is scheduler.current:
        return False
...
def testcase_leaks_references(leak_reason, soft_switching=None):
    ...
    def decorator(testcase):
        ...
        def wrapper(self):
            if soft_switching is not None and scheduler.enable_softswitch(None) != soft_switching:
...
def get_current_watchdog_list():
    ...
    scheduled = []
    t = scheduler.current.next
    while t not in (None, scheduler.current):
        scheduled.append(t)
        t = t.next
    for t in scheduled:
        t.remove()
    try:
        scheduler.run()  # creates the watchdog list
    finally:
        for t in scheduled:
            t.insert()
        if scheduled:
            assert scheduler.current.next == scheduled[0]
    watchdog_list = get_watchdog_list(-1)
...
class SwitchRecorder(object):
    def __init__(self, old_scb):
        self.ids = {}
        if scheduler.main is scheduler.current:
            self.ids[id(scheduler.main)] = "main/current"
        else:
            self.ids[id(scheduler.main)] = "main"
            self.ids[id(scheduler.current)] = "current"
        self.switches = []
...
class SchedulerTestCaseMixin(object):
    ...
    def trace_tasklet_switches(self):
        old_scb = scheduler.get_schedule_callback()
        switch_recorder = SwitchRecorder(old_scb)
        self.addCleanup(scheduler.set_schedule_callback, old_scb)
        self.addCleanup(switch_recorder.print)
        scheduler.set_schedule_callback(switch_recorder)

    def register_tasklet_name(self, tlet, name):
        scb = scheduler.get_schedule_callback()
        if isinstance(scb, SwitchRecorder):
            if isinstance(tlet, scheduler.tasklet):
                tlet = id(tlet)
...
class SchedulerTestCaseMeta(type):
    def __init__(cls, *args, **kw):
        cls.prepare_test_methods()


class SchedulerTestCase(unittest.TestCase, SchedulerTestCaseMixin, metaclass=SchedulerTestCaseMeta):
    ...
    def prepare_test_method(cls, func, name):
        ...
        def wrapper_hardswitch(self, method=func):
            ...
            self.assertFalse(scheduler.enable_softswitch(None), "softswitch is enabled")
            return method(self)
    ...
    def prepare_pickle_test_method(cls, func, name=None):
        ...
        if hasattr(func, "_pickle_created"):
            return SchedulerTestCase.prepare_test_method.__func__(cls, func, name)
        ...
        for i in range(0, pickle.HIGHEST_PROTOCOL + 1):
            for p_letter in ("C", "P"):
                def test(self, method=func, proto=i, pickle_module=p_letter, unpickle_module=p_letter):
                    self.assertTrue(self._SchedulerTestCase__setup_called,
                ...
                r.extend(SchedulerTestCase.prepare_test_method.__func__(cls, test, test.__name__))
        return r
    ...
    def __init__(self, methodName='runTest'):
        ...
        super(SchedulerTestCase, self).__init__(methodName=methodName)
    ...
    def tearDown(self):
        ...
        for t in [t for t in gc.garbage if isinstance(t, scheduler.tasklet)]:
    ...
    def tasklet_is_uncollectable(self, tlet):
        self.assertIsInstance(tlet, scheduler.tasklet)
    ...
    if sys.hexversion >= 0x3040000:
        def _addSkip(self, result, test_case, reason):
            ...
            super(SchedulerTestCase, self)._addSkip(result, test_case, reason)
    else:
        def _addSkip(self, result, reason):
            ...
            if hasattr(self, "_ran_AsTaskletTestCase_setUp"):
                # avoid a assertion violation in AsTaskletTestCase.run
                self._ran_AsTaskletTestCase_setUp = True
            super(SchedulerTestCase, self)._addSkip(result, reason)


_tc = SchedulerTestCase(methodName='run')
try:
    _tc.setUp()
    SchedulerTestCase.SAFE_TESTCASE_ATTRIBUTES = _tc.__dict__.keys()
finally:
    del _tc


class SchedulerPickleTestCase(SchedulerTestCase):
...
def restore_testcase_from_id(id_):
    return SchedulerTestCase.restore_testcase_from_id(id_)


class AsTaskletTestCase(SchedulerTestCase):
    ...
    def setUp(self):
        ...
        expected_thread_count = self.setUpStacklessTestCase()
        self.assertEqual(scheduler.getruncount(
        ), 1, "Leakage from other tests, with %d tasklets still in the scheduler" % (scheduler.getruncount() - 1))
        if withThreads:
            active_count = threading.active_count()
...
def get_reduce_frame():
    ...
    Only for testing!
    """
    return getattr(scheduler._scheduler._wrap, "reduce_frame", None)
