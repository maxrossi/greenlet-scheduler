...
from scheduler import schedule, tasklet

from support import SchedulerTestCase, SchedulerPickleTestCase, get_reduce_frame
...
class TestPickledTasklets(SchedulerPickleTestCase):
    ...
    def testSendSequence(self):
        ...
        c = scheduler.channel()
        t1 = scheduler.tasklet(sender)(c)
        t2 = scheduler.tasklet(receiver)(c)
        t1.run()
...
class TestFramePickling(SchedulerTestCase):
    ...
    def testLocalplus(self):
        ...
        def reduce_current():
            ...
            def func(current):
                result2.append(scheduler._scheduler._wrap.frame.__reduce__(current.frame))
            scheduler.tasklet().bind(func, (scheduler.current,)).run()
            return result[0]
        ...
        self.assertEqual(valid, int(scheduler.enable_softswitch(None) and
                                    scheduler.current.nesting_level == 0))
    ...
    def _test_setstate_prepare(self):
        ...
        def f1(bar="bar"):
            try:
                ...
                x = foo
                locals()
                return scheduler._scheduler._wrap.frame.__reduce__(sys._getframe())
            assert False
    ...
    def test_setstate_OK(self):
        ...
        wrap_frame = r[0](*r[1])
        self.assertIsInstance(wrap_frame, scheduler._scheduler._wrap.frame)
...
class TestDictViewPickling(SchedulerPickleTestCase):
...
class TestTraceback(SchedulerPickleTestCase):
...
class TestCoroutinePickling(SchedulerPickleTestCase):
    @types.coroutine
...
class TestAsyncGenPickling(SchedulerPickleTestCase):
    async def ag(self):
        yield 100
    ...
    def pickle_flags(self, preserve_finalizer=False, reset_finalizer=False):
        old_flags = scheduler.pickle_flags()
        new_flags = ((old_flags & ~(scheduler.PICKLEFLAGS_PRESERVE_AG_FINALIZER |
                                    scheduler.PICKLEFLAGS_RESET_AG_FINALIZER)) |
                     (scheduler.PICKLEFLAGS_PRESERVE_AG_FINALIZER if preserve_finalizer else 0) |
                     (scheduler.PICKLEFLAGS_RESET_AG_FINALIZER if reset_finalizer else 0))
        old_flags = scheduler.pickle_flags(new_flags)
        try:
            yield old_flags
        finally:
            scheduler.pickle_flags(old_flags)
...
class TestAsyncGenASendPickling(SchedulerPickleTestCase):
    async def ag(self):
        yield 100

    def inspect(self, ags):
        r = scheduler._scheduler._wrap.async_generator_asend.__reduce__(ags)
        self.assertIs(r[0], scheduler._scheduler._wrap.async_generator_asend)
...
class TestAsyncGenAThrowPickling(SchedulerPickleTestCase):
    async def ag(self):
        yield 100
        yield 200

    def inspect(self, ags):
        r = scheduler._scheduler._wrap.async_generator_athrow.__reduce__(ags)
        self.assertIs(r[0], scheduler._scheduler._wrap.async_generator_athrow)
        self.assertIsInstance(r[2][0], types.AsyncGeneratorType)
        return r[2][1:]
...
class TestContextRunCallbackPickling(SchedulerTestCase):
    ...
    def _test_pickle_in_context_run(self, ctx1):
        ...
        t = scheduler.tasklet(ctx2.run)(self.task)
        ...
        for f in frames:
            if not isinstance(f, scheduler.cframe):
                continue
    ...
    def _test_pickle_in_tasklet_context_run(self, ctx1):
        ...
        ctx2 = contextvars.Context()
        t_run = scheduler.tasklet().set_context(ctx2)
        t = scheduler.tasklet(t_run.context_run)(self.task)
    ...
    def test_tasklet_get_unpicklable_state(self):
        ...
        ctx = scheduler._tasklet_get_unpicklable_state(scheduler.current)['context']
...
class TestContextPickling(SchedulerPickleTestCase):
    # We can't pickle a contextvars.Context object.
    ...
    def setUp(self):
        super().setUp()
        scheduler.pickle_flags(scheduler.PICKLEFLAGS_PICKLE_CONTEXT, scheduler.PICKLEFLAGS_PICKLE_CONTEXT)

    def tearDown(self):
        super().tearDown()
        scheduler.pickle_flags(0, scheduler.PICKLEFLAGS_PICKLE_CONTEXT)
    ...
    def test_pickle_tasklet_with_context(self):
        ...
        t = scheduler.tasklet(self.task)("changed").set_context(ctx)
        scheduler.run()
        self.assertTrue(t.alive)
    ...
    def test_pickle_tasklet_in_tasklet_context_run(self):
        ctx1 = contextvars.copy_context()
        ctx2 = contextvars.Context()
        t = scheduler.tasklet(scheduler.tasklet().set_context(ctx2).context_run)(self.task, "changed").set_context(ctx1)
...
class TestCopy(SchedulerTestCase):
...
class TestPickleFlags(unittest.TestCase):
    def _test_pickle_flags(self, func):
        old = func()
        n_flags = sum(1 for k in dir(scheduler) if k.startswith('PICKLEFLAGS_'))
        m = 1 << n_flags
    ...
    def test_pickle_flags(self):
        self._test_pickle_flags(scheduler.pickle_flags)

    def test_pickle_flags_default(self):
        self._test_pickle_flags(scheduler.pickle_flags_default)

    def test_new_thread(self):
        def run():
            self.pickle_flags = scheduler.pickle_flags()

        current = scheduler.pickle_flags()
        old = scheduler.pickle_flags_default(0)
        self.assertEqual(scheduler.pickle_flags(), current)
        self.addCleanup(scheduler.pickle_flags_default, old)
        ...
        scheduler.pickle_flags_default(1)
        self.assertEqual(scheduler.pickle_flags(), current)
...
class TestCodePickling(unittest.TestCase):
    def test_reduce_magic(self):
        ...
        reduce = scheduler._scheduler._wrap.code.__reduce__
        reduced = reduce(code)
    ...
    def test_func_with_wrong_magic(self):
        ...
        reduce = scheduler._scheduler._wrap.function.__reduce__
...
class TestFunctionPickling(SchedulerPickleTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_unpickle_pre38(self):
        ...
        reduced = scheduler._scheduler._wrap.function.__reduce__(obj)
