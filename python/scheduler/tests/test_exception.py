...
from support import SchedulerPickleTestCase


class TestExcStatePickling(SchedulerPickleTestCase):
    ...
    def test_tasklet_pickle_with_exc(self):
        ...
        def task_inner():
            scheduler.schedule_remove((2, sys.exc_info()))
            try:
                raise e2
            except Exception:
                scheduler.schedule_remove((3, sys.exc_info()))
            scheduler.schedule_remove((4, sys.exc_info()))

        def task_outer():
            scheduler.schedule_remove((0, sys.exc_info()))
            try:
                raise e1
            except Exception:
                scheduler.schedule_remove((1, sys.exc_info()))
                task_inner()
                scheduler.schedule_remove((5, sys.exc_info()))
            scheduler.schedule_remove((6, sys.exc_info()))

        t = scheduler.tasklet(task_outer)()
        expected_exc = (
            (None, None),               # 0
    ...
    def test_generator_pickle_with_exc(self):
        ...
        def task_inner():
            ...
            try:
                ...
                yield 6
            yield 7
            scheduler.schedule_remove((8, sys.exc_info()))
        ...
        def task_outer():
            ...
            try:
                ...
                scheduler.schedule_remove((1, sys.exc_info()))
                for x in task_inner():
                    scheduler.schedule_remove((x, sys.exc_info()))
            scheduler.schedule_remove((9, sys.exc_info()))
