...
from support import SchedulerTestCase, get_reduce_frame
...
class TestExceptionState(SchedulerTestCase):
    def Tasklet(self):
        try:
            1 / 0
        except Exception:
    ...
    def testExceptionStatePickled(self):
        ...
        t.remove()
        t.bind(None)
        p = Unpickler(BytesIO(io.getvalue()))
...
class TestTracingState(SchedulerTestCase):
    def setUp(self):
    ...
    def testUnpickledTracingState2(self):
        ...
        saved = scheduler.pickle_with_tracing_state
        scheduler.pickle_with_tracing_state = True
        try:
            self._testTracingOrProfileState(do_pickle=True, do_trace=True)
        finally:
            scheduler.pickle_with_tracing_state = saved
    ...
    def testReduceOfTracingState(self):
        ...
        saved = scheduler.pickle_with_tracing_state
        scheduler.pickle_with_tracing_state = False
        import _scheduler
        try:
            reduced_tasklet1 = t.__reduce__()
            reduced_frame1 = _scheduler._wrap.frame.__reduce__(frame)
            scheduler.pickle_with_tracing_state = True
            reduced_tasklet2 = t.__reduce__()
            reduced_frame2 = _scheduler._wrap.frame.__reduce__(frame)
        finally:
            scheduler.pickle_with_tracing_state = saved
